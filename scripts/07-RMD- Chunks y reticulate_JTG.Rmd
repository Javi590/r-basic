...
b <- np_array(c(1:10), order="C") # Con __order=""__ Se le indica que para crearlo se utilice el estilo de guardado en memoria de **C** en lugar del estilo de forttrump, que es el que utiliza python para optimizar los calculos de muchos números. 
...
Y posteriormente, desde la chunk de python y con la función **r.** accedemos a cualquier módilo, función, variable, clase.. de r expuesta a python a trqavés de r_to_py:
...
sparse_mat <- sparseMatrix(
  ...
  x = runif(N), # número aleatorio uniforme entre 0 y N
  dims= c(N,N) #Dimensiones de la matriz
)

sparse_mat

# Ahora utilizamos la función R to py:
